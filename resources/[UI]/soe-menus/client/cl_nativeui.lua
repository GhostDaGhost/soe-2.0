-- SoE NativeUI
-- Originally made by FrazzIe
-- Edited by iTexZoz
-- MAJOR edits by GhostDaGhost
...
function MenuPool:AddSubMenu(Menu, Text, Subtitle, Description, KeepPosition, KeepBanner)
    if Menu() == "UIMenu" then
        local Item = UIMenuItem.New(tostring(Text), Description or "")
        Menu:AddItem(Item)
        local SubMenu
        if KeepPosition then
            SubMenu = UIMenu.New(Text, Subtitle, Menu.Position.X, Menu.Position.Y)
        else
            SubMenu = UIMenu.New(Text, Subtitle)
        end
        if KeepBanner then
...
function NativeUI.CreateColourPanel(Title, Colours, Value)
    return UIMenuColourPanel.New(Title, Colours, Value)
end

function NativeUI.CreatePercentagePanel(MinText, MaxText, MiddleText)
    return UIMenuPercentagePanel.New(MinText, MaxText, MiddleText)
end
...
UIProgressButton = setmetatable({}, UIProgressButton)

UIProgressButton.__index = UIProgressButton

UIProgressButton.__call = function()
    return "UIProgressButton", "UIProgressButton"
end

function UIProgressButton.New(Text, Description)
    local _UIProgressButton = {
        Rectangle = UIResRectangle.New(0, 0, 431, 38, 255, 255, 255, 20),
        Text = UIResText.New(tostring(Text) or "", 7, 0, 0.31, 245, 245, 245, 255, 0),
        _Text = {
            Padding = {X = 7},
            Colour = {
        ...
        Label = {
            Text = UIResText.New("", 0, 0, 0.31, 245, 245, 245, 255, 0, "Right"),
    ...
    return setmetatable(_UIProgressButton, UIProgressButton)
end

function UIProgressButton:Position(Y)
    if tonumber(Y) then
...
function UIProgressButton:RightLabel(Text, MainColour, HighlightColour)
...
function UIProgressButton:Text(Text)
    if tostring(Text) and Text ~= nil then
...
function UIProgressButton:Draw()
    self.Rectangle:Size(431 + self.ParentMenu.WidthOffset, self.Rectangle.Height)
...
function UIMenuItem.New(Text, Description)
    local _UIMenuItem = {
        Rectangle = UIResRectangle.New(0, 0, 431, 38, 255, 255, 255, 20),
        Text = UIResText.New(tostring(Text) or "", 3.5, 0, 0.33, 245, 245, 245, 255, 0),
        _Text = {
            Padding = {X = 3.5},
            Colour = {
                Selected = {R = 0, G = 0, B = 0, A = 255},
...
function UIMenuColourPanel.New(Title, Colours, Value)
...
function UIMenuPercentagePanel.New(MinText, MaxText, MiddleText)
    local _UIMenuPercentagePanel = {
        ...
        Text = {
            ...
            Max = UIResText.New(MiddleText or "Opacity", 0, 0, 0.35, 255, 255, 255, 255, 0, "Centre"),
            Title = UIResText.New(MaxText or "100%", 0, 0, 0.35, 255, 255, 255, 255, 0, "Centre")
        },
        Audio = {Slider = "CONTINUOUS_SLIDER", Library = "HUD_FRONTEND_DEFAULT_SOUNDSET", Id = nil},
        ParentItem = nil
    }
...
function UIMenuProgressItem.New(Text, Items, Index, Description, Counter)
    ...
    local _UIMenuProgressItem = {
        Base = UIProgressButton.New(Text or "", Description or ""),
        Data = {
            Items = Items,
        ...
        Background = UIResRectangle.New(0, 0, 415, 20, 0, 0, 0, 170),
...
function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName, Heading, R, G, B, A)
    ...
    if TxtDictionary ~= nil then
        TxtDictionary = tostring(TxtDictionary) or 0
    else
        TxtDictionary = 0
    end
    if TxtName ~= nil then
        TxtName = tostring(TxtName) or 0
    else
        TxtName = 0
    end
    if Heading ~= nil then
    ...
    local _UIMenu = {
        ...
        Banner = nil,
        Title = UIResText.New(Title, 215 + X, 20 + Y, 1.03, 255, 255, 255, 255, 1, 1, 0),
        BetterSize = false,
        Subtitle = {ExtraY = 0},
        WidthOffset = 0,
        Position = {X = X, Y = Y},
        DrawOffset = {X = 0, Y = 0},
        Pagination = {Min = 0, Max = 12, Total = 11},
        PageCounter = {
            isCustom = false,
            PreText = ""
        },
    ...
    if Subtitle ~= "" and Subtitle ~= nil then
        _UIMenu.Subtitle.Rectangle =
            UIResRectangle.New(0 + _UIMenu.Position.X, 107 + _UIMenu.Position.Y, 431, 37, 0, 0, 0, 175)
        _UIMenu.Subtitle.Text =
            UIResText.New(Subtitle, 3.5 + _UIMenu.Position.X, 110 + _UIMenu.Position.Y, 0.35, 245, 245, 245, 255, 0)
    ...
    _UIMenu.Description.Bar = UIResRectangle.New(_UIMenu.Position.X, 123, 431, 4, 0, 0, 0, 0)
    _UIMenu.Description.Rectangle = UIResRectangle.New(_UIMenu.Position.X, 127, 431, 30, 0, 0, 0, 200)
    _UIMenu.Description.Text = UIResText.New("Description", _UIMenu.Position.X + 5, 125, 0.34)
...
function UIMenu:GoBack()
    ...
    local currentSelection = self:CurrentSelection()
    currentSelection = self:GetItemAt(currentSelection)
