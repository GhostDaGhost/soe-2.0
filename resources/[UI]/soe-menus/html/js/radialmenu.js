...
window.addEventListener('message', (event) => {
    ...
    switch(data.type) {
        case 'Radial.Open':
            ...
            break;
        case 'Radial.Reset':
            ResetRadial(true, true);
            $.post("https://soe-menus/Radial.Close", JSON.stringify({}));
            break;
        default:
...
// RESET RADIAL MENU
function ResetRadial(fromLua) {
    if (fromLua) {
        console.log('[Radial Menu] UI Resetted.');
    }

    $("#radial-menu").remove();
    if (myRadialMenu != null) {
        myRadialMenu.destroy();
    }
}

// OPENS RADIAL MENU
function OpenRadialMenu(data) {
    ResetRadial(false);
    $("#radial-menu-container").append(`<div id="radial-menu"></div>`);

    myRadialMenu = new RadialMenu({
        parent: document.getElementById("radial-menu"),
        size: 455,
        ...
        onClick: (item) => {
            ...
            $.post("https://soe-menus/Radial.SliceClick", JSON.stringify({
                type: item.type,
                action: item.func,
...
$(() => {
    // CLOSES RADIAL MENU COMPLETELY WHEN 'TAB' IS PRESSED
...
function RadialMenu(params) {
    ...
    self.size      = params.size    || defaultSize;
    ...
    self.sectorCount = Math.max(self.menuItems.length, minSectors);
...
RadialMenu.prototype.createHolder = function () {
    ...
    self.holder.className = 'radial-menu-text';
...
RadialMenu.prototype.createMenu = function (classValue, levelItems, nested) {
    ...
    self.sectorCount = Math.max(self.levelItems.length, minSectors);
