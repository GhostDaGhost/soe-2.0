...
const emoji = {INIT_BEFORE_MAP_LOADED: ['🍉'], INIT_AFTER_MAP_LOADED: ['🍋', '🍊'], INIT_SESSION: ['🍐', '🍅', '🍆'],};
...
window.addEventListener("message", function (event) {
    (handlers[event.data.eventName] || function () {})(event.data);
});
...
function RefreshMyImage() {
    var url = 'https://soe.gg/loadscreen/';

    setInterval(() => {
        let randomID = new Date().getTime();
        $("#background").css("background-image", "url(" + url + "?random=" + randomID + ")");
    }, 5500)
}
RefreshMyImage();
...
const handlers = {
    ...
    startInitFunctionOrder(data) {
        count = data.count;
        printLog(1, String.format('[{0}] Running functions of order {1} ({2} total)', data.type, data.order,
            data.count));
        if (data.type) doProgress(data.type);
    },
    initFunctionInvoking(data) {
        printLog(3, String.format('Invoking {0} {1} init ({2} of {3})', data.name, data.type, data.idx,
            count));
        if (data.type) doProgress(data.type);
    },
