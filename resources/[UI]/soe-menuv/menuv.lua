...
local SEND_NUI_MESSAGE = function(input)
    exports['soe-menuv']:SendNUIMessage(input)
end
...
exports['soe-menuv']:IsLoaded(function()
    MenuV.Loaded = true
end)
...
REGISTER_NUI_CALLBACK('update', function(info, cb)
    ...
    for k, v in pairs(MenuV.CurrentMenu.Items) do
        if (v.UUID == uuid) then
            ...
            elseif (v.__type == 'slider') then
                newValue = v.Values[Utilities:Ensure(info.now, 0) + 1].Value
                oldValue = v.Values[Utilities:Ensure(info.prev, 0) + 1].Value
            end
