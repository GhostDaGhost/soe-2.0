...
export default Vue.extend({
  ...
  mounted() {
    post("https://soe-chat/loaded", JSON.stringify({}));
    ...
    this.listener = (event: MessageEvent) => {
      ...
      const typeRef = item.type as
        ...
        'ON_SUGGESTION_REMOVE' | 'ON_TEMPLATE_ADD' | 'ON_UPDATE_THEMES' | 'ON_MODE_ADD' | 'ON_MODE_REMOVE' | 'hideInput' | 'Chat_ChangeSize';
  ...
  methods: {
    ...
    Chat_ChangeSize({size}: {size: string}) {
      $('.chat-messages').css('font-size', `${size}vh`);
    },
    ON_OPEN() {
      this.showInput = true;
      this.showWindow = true;
    ...
    hideInput(canceled = false) {
      ...
      if (canceled) {
        post("https://soe-chat/chatResult", JSON.stringify({ canceled }));
      }
      this.message = "";
