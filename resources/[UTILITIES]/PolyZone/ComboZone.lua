...
local function _circleRectCollide(circleX, circleY, radius, rectX, rectY, rectWidth, rectLength)
    ...
    if circleX < rectX then
        testX = rectX -- test left edge
    elseif circleX > rectX + rectWidth then
        testX = rectX + rectWidth
    end -- right edge
    if circleY < rectY then
        testY = rectY -- top edge
    elseif circleY > rectY + rectLength then
        testY = rectY + rectLength
    end -- bottom edge
        ...
        function()
            ...
            while not self.destroyed do
                if not self.paused then
                    ...
                    local isDifferent, enteredZones, leftZones =
                        _differenceBetweenInsideZones(insideZones, newInsideZones)
